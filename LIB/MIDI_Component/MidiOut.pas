...
// ** Added some modifications for Windows 7, 64bit  (2011-10-03, by Silhwan Hyun)

interface

uses
  SysUtils, Windows{WinTypes, WinProcs}, Messages, Classes, Controls, {Forms,} MMSystem, MidiCommon;
...
type
  ...
  protected
    ...
    FDeviceID: UIntPtr;
    {$else}
    FDeviceID: LongWord; { MIDI device ID }
    {$ifend}
 ...
 { Stuff from midioutCAPS }
    ...
    {$if CompilerVersion > 21} // Delphi XE or later
    procedure SetDeviceID(DeviceID: UIntPtr);
    {$else}
    procedure SetDeviceID(DeviceID: LongWord);
    {$ifend}
 ...
 { Methods }
    ...
    function GetVolume: LongWord;  // ** Added
 ...
 { TODO: Property editor with dropdown list of product names }
   ...
   {$if CompilerVersion > 21} // Delphi XE or later
    property DeviceID:UIntPtr read FDeviceID write SetDeviceID default 0;
   {$else}
    property DeviceID:LongWord read FDeviceID write SetDeviceID default 0;
   {$ifend}
...
{$if CompilerVersion > 21} // Delphi XE or later
procedure Tmidioutput.SetDeviceID(DeviceID: UIntPtr);
{$else}
procedure Tmidioutput.SetDeviceID(DeviceID: LongWord);
{$ifend}
var
...
begin
  if FState = mosOpen then
    ...
    if (DeviceID >= midioutGetNumDevs) and (DeviceID <> MIDI_MAPPER) then
      raise EmidioutputError.Create('Invalid device ID #'+Inttostr(DeviceID))
    else
    begin
      FDeviceID := DeviceID;
  ...
  machine. The FProductname property will still be set by SetDeviceID }
    ...
    { Loop uses -1 to test for MIDI_MAPPER as well }
      ...
      if FProductName <> NewProductName then
        ...
        {$if CompilerVersion > 21} // Delphi XE or later
        SetDeviceID(UIntPtr(testDeviceID));
        {$else}
...
function TMidiOutput.GetVolume: LongWord;   // ** Added
var
  dwVolume: DWORD;
begin
  dwVolume := 0;
...
begin
  ...
  else if Message.Msg = WM_QueryEndSession then  // ** Query message at system shutdown
    Message.Result := 1;                         // ** Allow system shutdown
 // ** Add followings to render normal Result value for unhandled window messages.
 // else
...
procedure Tmidioutput.SentAllNotesOff;
